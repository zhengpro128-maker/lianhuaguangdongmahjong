...
const rules = computed(() => {
  if (props.variant === 'wuhan-huanghuang') return [
    ...
    ['红中与癞子杠', '红中和癞子均不可作为普通弃牌；直接打出时立即按单张杠亮出并从牌墙尾补摸，因此不会出现额外的杠按钮。两者均不能被他家吃碰杠。'],
    ['胡牌限制', '结算胡分必须达到 9 分才能胡。屁胡最多使用 1 张癞子；碰碰胡、清一色、门前清、七对、龙七对、双龙七对等大胡可使用更多癞子。发财、白板可正常参与自摸、点炮和抢杠胡判定。'],
    ['特殊胡法', '门前清单算 6 分，与其他大胡叠加时按 ×2 计入牌型底分，且必须自摸或抢杠胡才成立；支持全求人、杠上开花和抢杠胡。将一色、风一色、见字胡不计大胡；弃牌响应采用单响截胡。'],
    ['计分', '自摸屁胡从 3 分起算，胡家杠番先计入基础分；每名付款者再按自己持有的杠独立翻倍。自摸大胡在大胡牌型分上 ×1.5（门前清、杠上开花不重复计算）。点炮时三家均付款：七对、清一色、碰碰胡的放炮者按基础应付分 ×1.2，其他点炮按 ×2；其余两家不翻倍。硬胡 ×2；红中杠、直杠、补杠 ×2，暗杠和癞子杠 ×4；每名付款者最多支付 50 分。'],
    ['荒庄', '牌墙剩余 8 张时停止摸牌并荒庄。'],
  ]
  if (props.variant === 'lotus-blood-flow') return [
    ...
    ['杠与场制', '杠牌只改变副露并补摸，不作即时收付。东风 4 局、半庄 8 局，局末轮庄，无庄家倍率或买马。'],
  ]
  if (props.variant === 'lotus-legacy') {
    return [
      ...
      ['杠牌计分', '加杠、明杠、暗杠和风杠均不即时收付分数；仅保留开杠后的补摸流程。'],
