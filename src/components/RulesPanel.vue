...
const rules = computed(() => {
  if (props.variant === 'wuhan-huanghuang') return [
    ...
    ['胡牌限制', '结算胡分必须达到 10 分才能胡。屁胡最多使用 1 张癞子；碰碰胡、清一色、门前清、七对、龙七对、双龙七对等大胡可使用更多癞子。手中见发财或白板只能自摸或抢杠胡。'],
    ['特殊胡法', '门前清按 6 分起算并视为大胡；支持全求人、杠上开花和抢杠胡。将一色、风一色、见字胡不计大胡；弃牌响应采用单响截胡。'],
    ['计分', '自摸屁胡从 3 分起算，杠番继续翻倍；自摸大胡在大胡牌型分上 ×1.5。点炮时三家均付款，放炮者额外支付 2 分，大胡点炮的牌型分 ×1.2。硬胡 ×2；红中杠、直杠、补杠 ×2，暗杠和癞子杠 ×4；每名付款者最多支付 50 分。'],
    ['荒庄', '牌墙剩余 8 张时停止摸牌并荒庄。'],
  ]
...
const baseNote = computed(() => props.variant === 'lotus-blood-flow' ? '底分 10 · 单家每次最多 640 分 · 牌墙耗尽结束本局' : props.variant === 'lotus-legacy'
  ...
  : props.variant === 'wuhan-huanghuang' ? '起始 1000 分 · 10 分起胡 · 每名付款者 50 分封顶 · 余 8 张荒庄' : `基础分 ${BASE_SCORE} 分 · 总分 = 底分 × 倍数 + 中马数 × 底分`)
</script>
