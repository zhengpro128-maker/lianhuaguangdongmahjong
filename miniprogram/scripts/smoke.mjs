...
try {
  ...
  await tap({ type: 'match', value: 'rounds16' })
  await page.waitForFunction(() => window.mini.snapshot().settings.matchType === 'rounds16')
  await page.screenshot({ path: path.join(evidence, 'mini-local.png') })
  await tap({ type: 'match', value: 'rounds8' })
  await page.waitForFunction(() => window.mini.snapshot().settings.matchType === 'rounds8')
  await tap({ local: 'rules' })
  await page.screenshot({ path: path.join(evidence, 'mini-rules.png') })
  await tap({ local: 'close' })
  await tap({ type: 'match', value: 'rounds4' })
