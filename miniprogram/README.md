...
- 入口选择「单机模式」或「联机模式」。单机选择 4 局、8 局、16 局，与三个本地 AI 对局。每次胡牌或荒庄都计一局，连庄保留庄位但计入局数；联机创建房间也使用相同的三档局数。
...
入口的「联机模式」卡片覆盖预先创建的透明 `wx.createUserInfoButton` 原生按钮，点击卡片直接授权并获取头像昵称，再调用 `wx.login` 和服务端身份校验。两步都成功后自动进入联机大厅，展示创建房间、加入房间和房间列表，不再要求点击独立的登录按钮。取消授权、缺少头像昵称或服务器失败均提示失败，不自动回退为默认资料；建房/加入/重进也不绕过此入口。已授权用户是否再次出现微信授权弹窗由微信控制，不能承诺每次都弹窗。登录 token 仅保存在内存中，重启后重新授权登录；房间重进凭证仍保存在微信本地存储。真实资料返回须用该 AppID 的手机体验版验证。
...
4. 先部署本次后端：新增 `rounds4` / `rounds8` / `rounds16` 协议值，并在启动时兼容迁移 SQLite / PostgreSQL 的房间模式约束，保留旧房间和战绩。然后在 master 构建：`VITE_API_BASE=https://你的后端域名 pnpm build:mini`。也可复用本机 `.env.wechat.local` 中的 `VITE_WECHAT_API_BASE`；显式 `VITE_API_BASE` 优先。两者均未配置时联机入口显示明确提示，不会请求示例服务器。随后在开发者工具重新编译。此构建使用 master 的 WebSocket 源码；vibehub 仅同步小游戏成品，不在 P2P 分支重建小游戏。
