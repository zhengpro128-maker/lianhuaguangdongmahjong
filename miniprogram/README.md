...
大厅新增「微信登录」「创建房间」「加入房间」，支持准备、房主开局、空座 AI 补位、断线重连和重进房间。仅创建和加入武汉晃晃房间，主题固定墨玉。联机复用 master 的 WebSocket 客户端和 Python 权威规则，出牌/吃碰杠胡由服务端验证；本地只做显示和输入。

「微信登录」直接调用 `wx.login` 并与服务器交换身份凭证，显示登录结果。登录后可点击「头像昵称」，再点击原生按钮申请平台允许返回的玩家资料。拒绝资料授权仍可使用默认资料。注意：`wx.login` 不提供头像昵称；新版本微信可能仅返回匿名资料，本项目不能绕过平台限制取得真实微信资料。资料会在本机缓存用于再次登录，登录 token 仅在内存中保存；小游戏重启后重新换取 token，房间重进凭证保存在微信本地存储。
