...
「微信登录」先显示原生资料授权按钮，玩家点击后获取平台允许返回的头像昵称，再通过 `wx.login` 登录。拒绝资料授权仍可使用默认资料。注意：`wx.login` 不提供头像昵称；新版本微信可能仅返回匿名资料，本项目不能绕过平台限制取得真实微信资料。资料会在本机缓存用于再次登录，登录 token 仅在内存中保存；小游戏重启后重新换取 token，房间重进凭证保存在微信本地存储。
